...
public class NotificationRestController {
    ...
    @RequestMapping("/test/getlastnotifications")
    public ResponseEntity<ArrayList<Notification>> getLastNotifications(@RequestParam int id) throws Exception {
        ArrayList<Notification> list = new ArrayList<>();

        try{
            list = (ArrayList<Notification>) repository.getNotificationUser(id);
            return new ResponseEntity<>(list, HttpStatus.OK);
        }catch(Exception e){
            return new ResponseEntity<>(null,HttpStatus.UNAUTHORIZED);
        }
